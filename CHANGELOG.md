...
`prompts/` は検品担当の指示文を1か所だけ変えました（下の「検品の指摘」）。
**公開される投稿の文面は変わりません。**更新の仕組みの貼り替えも要りません。

🔴 **「最近の動き」が別物になります。**下の1件目を読んでください。

### Changed

- 🔴 **「最近の動き」を、内部の記録の垂れ流しから、意味のあるできごとに変えました。**

  これまでは、システムの内部記録を12件そのまま出していたので、
  `Draft drf_5471… cleared inspection (smell 17).` のような**英語と内部IDが並んでいました。**
  ライセンシーには読めず、どれもニュースではありませんでした。

  これからは、次のものが日本語で出ます。

  - **クリックが入りました** ／ **成果が入りました（確定前）** ／ **確定報酬が入りました**
  - **投稿を公開しました**（自分で手で投稿したものも含む）／ **あなたが投稿する番です**
  - **あなたが決めたこと**（何件のうち何件を承認したか、見送ったか）
  - **アカウントを止めた・再開した**
  - 🔴 **うまくいかなかったこと**（何の段階で止まったか、理由つき）

  **失敗は、内部的なものでも必ず出します。**失敗が画面に出ないと、あるアカウントが
  何日も動いていないのに気づけないためです。**専用の文言が無い失敗にも**
  「うまくいかなかったことがあります。詳しくは記録にあります。」の1行が出ます。

  **内部の記録そのものは、これまでどおり全部残ります。**変えたのは画面に出す範囲だけです。

- **全アカウントの表を、ほかの節と同じ幅に揃えました。**表だけが左右にはみ出して見えていました。
  入りきらない列（確定報酬・型・計測）は、表の中だけ横にスクロールします。
  **スクロールできることが分かるように、細いスクロールバーを常時表示**します。
  ⚠️ お使いのブラウザによっては、スクロールバーが操作するまで見えないことがあります。

- **表の「開く」を、アカウント名のすぐ右に移しました。**これまでは右端にあり、
  スクロールしないと押せませんでした。

- **確定報酬を、通貨ごとに1行で表示します。**複数の通貨があるとき、
  `1,234 JPY / 567 USD` と1行につなげて出していました。**通貨をまたいで合算しない**
  という約束は変わりません。

- **「数字と名前」の並びを、ラベルが上・数字が下に統一しました。**「今日」の画面と
  アカウントの画面で、同じ数字が逆の順に並んでいました。

- **判断待ちの枠に、緑の縁取りを付けました**（あなたが動かないと進まないものだけ。
  期限が過ぎたものには付きません）。

### Fixed

- 🔴 **検品の指摘が、書き直す前の下書きについてのまま残ることがありました。**
  実際にあった例：本文が「わたしは…」で始まっているのに、
  「『わたし』が一度も出てこない」という指摘が付いていました。
  書き直しで直ったあとも、直る前の下書きの指摘が画面に残っていたためです。

  指示文を「書き直したあとの文面について」と明記し、さらに、**指摘が引用している文が
  書き直し後の本文に無ければ、その指摘を落とす**仕組みを入れました。
  ⚠️ 特定の一文を指さない指摘（「〜が一度も出てこない」のような）は、この仕組みでは
  確かめられません。そちらは指示文の変更だけが頼りです。

- **投稿の承認カードに、内部の時刻表記と英語がそのまま出ていました。**
  正しい表記（`2026-09-26 07:30（日本標準時）`）のすぐ隣に、
  `2026-09-25T22:30:14.666Z — 07:30 Asia/Tokyo - default slot - …` と出ていました。
  重複していたので、後者を出さないようにしました。
...
- 🔴 **何も待っていない日、トップ画面はアカウント名を並べるだけで、何も言っていませんでした。**

  つまり「**今日は異常ありません**」と「**確かめられませんでした**」が、
  画面の上で**まったく同じに見えていました。**

  これは危ない形です。このプラットフォームは以前、**あるアカウントが2日間まったく動いて
  いなかった間、画面がずっと何も言わなかった**ことがあります。トップ画面から判断待ちを
  外した（v0.12.0）のは、**この一覧が代わりに知らせるという前提**の上でした。

  各アカウントの行が、いまこう言います。

  - **今日は完了しました**（次に動く時刻も添えます）
  - **今日はまだ動いていません** ← 見に行ってほしいのはこれです
  - 実行中です ／ 非アクティブです ／ 停止しています ／ 設定で無効です

  **「今日はまだ動いていません」のときだけ、次回の時刻を出しません。**
  出すと、**いちばん見てほしいアカウントに「大丈夫そう」という印象**を与えてしまうためです。

  あわせて、**止めたアカウントが「失敗しています」と出続ける**ことがなくなりました。
  非アクティブ・停止中であることが優先されます。

- **画面の待ち時間を、もう一段短くしました。**

  v0.12.1 で並べ替えた結果、**5.3秒 → 3.6秒**にはなっていましたが、**同じ情報を何度も
  取り直す**のをやめられていませんでした。原因は、覚えておくための目印が
  **リクエストごとに作り直されていた**ことです（同じ間違いを、3か所で繰り返していました）。

  あわせて、起動時のデータベース準備を**7往復から2往復**にまとめました。
...
- 🔴 **アカウントを切り替えても、前のアカウントの画面が数秒そのまま残っていました。**

  画面は押した瞬間に切り替わるのに、**中身が入れ替わるのは、情報を取り終えたあと**でした。
  その間、**前のアカウントの内容を見ていることになります。**しかもそのボタンは、
  前のアカウントを指していました。

  さらに悪いことに、**これは判断を狂わせます。**「このアカウントのサイクルが終わらない」と
  見えていたものが、実際には終わって投稿まで済んでいた、ということが起きていました。

  移動したときは、**情報を取りに行く前に、画面をいったん空にする**ようにしました。
  30秒ごとの自動更新では空にしないので、**画面がちらつくことはありません。**

- 🔴 **画面の待ち時間を短くしました。**

  実測で、トップ画面が **5.2秒**、アカウントの画面が **3.4秒** かかっていました。
  切り替えると、合わせて**9秒近く**待つことになります。

  原因は、**やらなくてよい往復を毎回していた**ことと、**できるはずの並行処理を
  順番に待っていた**ことでした。

  - **毎回、データベースの準備確認をやり直していました**（本来は起動時に1回で済むもの）
  - **アカウントごとの集計を1つずつ順番に**待っていました。
    **アカウントが増えるほど、比例して遅くなる**形でした
  - 画面が必要とする2つの情報を、**順番に**取りに行っていました

  すべて並行にし、やり直しをやめました。**アカウントが何個あっても、待ち時間はほぼ変わりません。**

- **うまくいかなかったときに、英語のエラーがそのまま出ることがありました。**
  モデルの利用が拒否されたとき（403）と、設定したモデルが見つからないとき（404）が
  訳されていませんでした。日本語で、何を確認すればよいかを出すようにしました。

- **トップ画面で、アカウントの比較表だけが横にはみ出して見えていました。**
  表の幅が、まわりの節と揃うようにしました。
...
🔴 **毎朝いちばん最初に見る画面が変わります。**下の1件目を読んでください。

### Changed

- 🔴 **トップ画面が「どのアカウントが、どんな状態か」だけになりました。**

  これまでは、全アカウントの判断待ちがトップに1本のリストで混ざって並んでいました。
  アカウントが増えるほど読みにくくなり、**どれがどのアカウントの話か**を毎回たどる必要がありました。

  これからは、トップには**アカウントごとの行**が出ます。各行が3つのことを言います。

  - **承認が要る（件数）**
  - **いま投稿する番です** — 強く出ます
  - **失敗しています**

  行を押すと、そのアカウントの画面に入ります。**判断待ち・予約中の投稿・手渡しの投稿は、
  そこにあります。**トップから消えたのではなく、**そのアカウントの場所へ寄りました。**

  **「いま投稿する番です」だけ強く出るのには理由があります。**承認はあなたが決めるまで待ちますが、
  **手渡しの投稿は待ちません。**枠の時刻に出さなければ、その枠は過ぎます。

  トップに残るもの：アカウントごとの状態、全アカウントの比較表、探索の提案、
  全アカウント合計の数字、最近の動き。

  **アカウントが1つでも同じ画面です。**いずれ増えることを前提に、手順を1つにしました。

### Fixed

- 🔴 **あるアカウントの実行中に別のアカウントで実行すると、最初のアカウントの
  「動かしています…」が消えていました。**

  ボタンが押せる状態に戻るので、**まだ走っている実行に2本目を送れてしまいました。**
  実行中かどうかを、アカウントごとに覚えるようにしました。

- **「この日を見る」で開いた内容が、別のアカウントの画面に残ることがありました。**
  アカウントを移ると、前のアカウントのサイクルの中身が、移った先の履歴の下に描かれていました。

- **設定画面の「いま動いている版」を「いま動いているバージョン」にしました。**

### 内部の変更（画面は1バイトも変わりません）

- 画面を組み立てているファイルが、2337行の1つから、77行＋14ファイルに分かれました。
  **出来上がるページが1バイトも変わらないこと**を確かめてあります。
  今後この画面の修正が速くなります。
